...
export async function POST(request: NextRequest) {
  ...
  **Output 1: \`ai_generated_prompt\` - The System Prompt**
  ...
  -   **Goal:** It must state the primary goal (e.g., "...your task is to convert a raw meeting transcript into structured minutes...").
  ...
  -   **Content:** This should not be a system prompt that instructs the ai to generate a summary but instead a prompt that explains how to replicate the user's minutes style with each and every specific.

  **Output 2: \`ai_generated_template\` - The Structural Template**
  This is the most critical part. The template must be a **structurally identical replica** of the example minutes, with only the specific data values replaced by placeholders.
